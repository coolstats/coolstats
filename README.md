...
`warperia`, whose repository root is shaped like the install ZIP. The source
and release workspace remains on `main`. If the launcher creates only one
`Interface/AddOns/coolstats/` folder containing `cache_addon/`, `realm_data/`,
`.gitignore`, or repository docs, Warperia is pointed at the source branch
instead of the generated install branch.
