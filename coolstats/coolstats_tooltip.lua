...
function coolstats.GetUwUHistoricalOverall(player)
	...
	return historical[1], historical[2], historical[3]
end
...
local function BuildUwUTooltipCache(player)
	...
	local historicalScore, historicalRank, historicalSpecIndex = coolstats.GetUwUHistoricalOverall(player)
	if historicalScore then
		...
		cache.historical = {
			...
			coolstats.FormatUwUHistoricalOverall(player, historicalScore, historicalRank, historicalSpecIndex),
			0.70, 0.70, 0.70,
...
RenderUwUPanel = function(panel, name, player, subtitle, guildName)
	...
	if historicalScore then
		local historicalRed, historicalGreen, historicalBlue = GetUwUScoreColor(historicalScore)
		AddInspectPanelLine(state, coolstats.GetUwUHistoricalOverallLabel(), coolstats.FormatUwUHistoricalOverall(player, historicalScore, historicalRank, historicalSpecIndex), historicalRed, historicalGreen, historicalBlue)
	end
...
if type(coolstats) == "table" then
	...
	function coolstats.GetCachedPlayerBrowserRowDisplay(row, panel)
		...
		local key = tostring(row.key or row.name or "")
			...
			.. "\030" .. tostring(row.phase2ScoreCenti or "") .. ":" .. tostring(row.phase2Rank or "") .. ":" .. tostring(row.phase2SpecIndex or "")
	...
	coolstats.CHANGELOG_ENTRIES = {
		...
		},
		{
			...
			notes = {
				"Refreshed all Warmane UwU data with realm-aware weekly caps: Onyxia 600, Lordaeron 1000, Icecrown 1500 per class/spec.",
				"Updated dynamic player chunks: Onyxia 17,836 players, Lordaeron 15,222 players, Icecrown 34,846 players.",
				"Ran duplicate-name verification and targeted boss-row repair during the weekly refresh.",
		...
		},
		{
			...
			notes = {
				...
				"Rebuilt the Warperia install branch from the release ZIP so launcher installs keep the multi-addon shard layout.",
			},
		},
		{
			version = "0.2.38",
			date = "2026-08-04",
			notes = {
				"Added the first-run feature guide with character-aware completion, tell sounds, and a pulsing welcome glow.",
				"Expanded guide callouts across browser filters, Statistics, Update Center, changelog, UwU panels, cached gear/talents, Log Analysis, sharing, and memory controls.",
				"Fixed first-run guide step wiring for panel opens and close-button progression.",
				"Fixed Armory and plain-text summary copy dialogs so they open above UwU panels.",
			},
		},
		{
			version = "0.2.37",
			date = "2026-08-03",
			notes = {
				"Refreshed all Warmane UwU data with realm-aware weekly caps.",
		...
		},
		{
			...
			notes = {
				...
				"Made boss dropdowns realm-aware for Warmane phase differences.",
			},
		},
	}
	...
	coolstats.INTRO_GUIDE_STEPS = {
		...
		},
		{
			...
			body = "Left-click a player to open their logs and cached gear. Right-click a row for compare tools, whisper/invite, cached talents, favourites, Armory, and links.",
